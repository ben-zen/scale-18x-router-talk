...
\begin{frame}[label={sec:org94796ed}]{Rackmount}
  ...
  \item<2-> Build your own
    \begin{figure}[htbp]
      ...
      \caption{A custom-built Supermicro-based router.}
    \end{figure}
...
\begin{frame}{\texttt{firewalld} zones}
  \begin{figure}
    \include{./assets/MultiZoneNetwork}
...
\begin{frame}{Open ports for services}
  We're going to be serving some services through
\begin{verbatim}
firewall-cmd --zone=internal \
  --add-service=dns,dhcp
...
\begin{frame}[fragile,label={sec:orgc73d656}]{Port forwarding for services}
  Single-port, one-to-one NAT for a web server behind our firewall
...
firewall-cmd --zone=external \
  --add-forward-port=port=80:\
      proto=tcp:toport=80:toaddr=172.24.3.2
firewall-cmd --zone=external \
   --add-forward-port=port=443:\
      proto=tcp:toport=443:toaddr=172.24.3.2
\end{verbatim}
...
\begin{frame}{DHCP and DNS}
  We're going to use \texttt{dnsmasq} for two key functions, and we'll use
  NetworkManager's version to run it.
\end{frame}
\end{document}
